...
## 1. Introduction
In Practical 1 and Practical 2 we studied fully-connected neural networks and how to implement them using *numpy* routines and Google's open-source Deep Learning framework called [TensorFlow](https://www.TensorFlow.org/). In this practical, we will go further and we will work with [Convolutional Neural Networks (CNNs)](http://neuralnetworksanddeeplearning.com/chap6.html) which are considered to be a more natural choice for images than fully-connected neural networks.

In Practical 3, the main goal is to get your familiar with CNNs and how you can use them. You will develop basic CNN and extend it to a siamese variant. We will also use [VGG-16](http://www.robots.ox.ac.uk/~vgg/research/very_deep/) model for transfer learning task on CIFAR10 dataset.

As in Practical 2, you will need to write a report about experiments and results. Section **Prerequisites**, **Tutorials** and **Troubleshooting** are almost the same as in Practical 2 and kept here for convenience purposes.

## 2. Prerequisites
In this practical, you will need to use GPU to train CNNs. You can use your own machine or SURF Sara to do it. There is the same instruction about how to install TensorFlow on your machine as in Practical 2.
  ...
  **You need to implement the following methods in this file.**
  1. Data sampling
      ...
      def next_batch(self, batch_size, fraction_same=0.1):
        ...
        """  
        ```
    This is a data generation utility as in ***cifar10_utils.py***. Its goal is to fetch you a minibatch from the training set to be fed to your model. Unlike standard Stochastic Gradient Descent (SGD) procedure (random sampling). We provide you with an idea of how in  ***cifar10_siamese_utils.py*** file, however, you will do a small research to come up with alternative (possibly better) methods. The output of this methods is three *numpy* arrays with 4, 4 and 1 dimensions respectively. More details can be found in the file documentation.
  2. Data creation
    ```python
...
### Task 1: CIFAR10  [20 points total]
We are going to build a CNN model using TensorFlow. Unlike previous practicals, you are given a fixed network to work with. This task involves the implementation of a lightweight class, namely `ConvNet() `. In class `ConvNet()` you need to complete the methods:
...
***Hint:*** You should expect a classification performance at least 70% with this plain convolutional  neural network.
...
***Hint***: Make sure to call `assign_ops` from `vgg.py` after initialising variables.
